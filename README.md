...
Binary Thrift with Kerberos uses the built-in pure-Python GSSAPI (no `sasl`/`thrift-sasl` required). Suitable for Kyuubi or HiveServer2 with `transportMode=binary`.

```py
from pycquery import hive
from pycquery_krb.common.conf import KerberosConf

conn = hive.Connection(
    host='kyuubi-host.example.com',
    port=11000,
    auth='KERBEROS',
    username='user@REALM',
    kerberos_service_name='hive/hostname',
    keytab_file='/path/to/user.keytab',
    krb_conf=KerberosConf.from_file('/path/to/krb5.conf'),
    service_mode='binary',
    database='default',
